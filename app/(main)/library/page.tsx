...
import { BookOpen, Download, Book, Loader2 } from "lucide-react";
import { useLanguage } from "@/context/LanguageContext";

// Define the Book type to match your Prisma schema
type BookData = {
  id: string;
  title_ar: string;
  title_en: string;
...
export default function LibraryPage() {
  ...
  // Fetch books on the client side to remain compatible with useLanguage
  useEffect(() => {
    async function fetchBooks() {
      try {
        const response = await fetch("/api/books"); // We will create this simple API route next
  ...
  const t = {
    ...
    download: isAr ? "تحميل PDF" : "Download PDF",
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-5 gap-6">
          {books.length === 0 ? (
            ...
            books.map((book) => (
              <div
                key={book.id}
                className="group bg-[#1A202C] border border-white/5 rounded-2xl p-4 hover:border-school-gold/50 shadow-lg hover:shadow-[0_0_30px_rgba(212,175,55,0.15)] transition-all duration-300 hover:-translate-y-2"
              >
                ...
                <div className="relative aspect-[3/4] bg-black/40 rounded-xl overflow-hidden mb-4 border border-white/5 shadow-inner">
                  ...
                  <div className="absolute inset-0 bg-school-dark/80 opacity-0 group-hover:opacity-100 transition-opacity duration-300 flex flex-col items-center justify-center backdrop-blur-[2px]">
                    <a
                      ...
                      download
                      className="bg-school-gold text-school-dark px-5 py-2 rounded-full font-bold text-xs flex items-center gap-2 hover:bg-white hover:scale-105 transition-all transform translate-y-4 group-hover:translate-y-0 duration-300 shadow-xl"
                    >
